...
standard_legend = {
    0: "No data", 1: "Tree covered", 2: "Grassland",
    3: "Cropland", 4: "Wetland", 5: "Artificial", 6: "Other land", 7: "Water body"
}

class_colors = {
    0:  "#000000", 1: "#4a7c46", 2:  "#be8729", 3:  "#ffd03a",
    4:  "#00e8b8", 5:  "#db1c21", 6:  "#c8d5cd", 7:  "#3042e1"
}
...
def load_sankey_params(s_data, t_data, mask, count_limit = 10):
    ...
    labels = (
        [standard_legend[cls] for cls in uniq_source] +
        [standard_legend[cls] for cls in uniq_target]
    )
    node_colors = (
        [class_colors[cls] for cls in uniq_source] +
...
def heterogeneity_func(window):
    ...
    return len(np.unique(window))  # number of unique classes, 1 = pure homogeneous, bigger means more landtypes within the window
