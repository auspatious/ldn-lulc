...
def get_country_boundary(country: str, boundaries_path: str = "../data/gadm_boundaries", level: int = 0) -> gpd.GeoDataFrame:
...
def sids_bounding_boxes(boundaries_path: str = "../data/gadm_boundaries", country: Optional[str] = None, buffer_m: float = 0) -> List[Tuple[float, float, float, float]]:
    ...
    """
    def _buffer_bounds(gdf: gpd.GeoDataFrame, meters: float) -> Tuple[float, float, float, float]:
        if buffer_m ==0: return tuple(gdf.total_bounds.tolist())
        orig_crs = gdf.crs
